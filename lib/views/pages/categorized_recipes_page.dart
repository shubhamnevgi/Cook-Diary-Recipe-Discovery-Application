...
class CategorizedRecipesPage extends StatefulWidget {
  final String category;
  const CategorizedRecipesPage({super.key, required this.category});

  @override
  State<CategorizedRecipesPage> createState() => _CategorizedRecipesPageState();
}

class _CategorizedRecipesPageState extends State<CategorizedRecipesPage> {
  Future<List<dynamic>> fetchByCategory(String category) async {
    final encoded = Uri.encodeComponent(category);
    final url = Uri.parse('https://www.themealdb.com/api/json/v1/1/filter.php?c=$encoded');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<dynamic>>(
        future: fetchByCategory(widget.category),
